...
app.config(['$routeProvider', function($routeProvider) {
    ...
    $routeProvider.when('/editor/quiz/edit/:type', {
        templateUrl: './views/editor-quiz-edit.html',
        controller: 'EditorQuizEditController'
    });
...
app.controller('EditorQuizController', ['$scope', '$routeParams', '$http', 'domainName', '$location', 'editorData', function($scope, $routeParams, $http, domainName, $location, editorData) {
    ...
    $scope.openEditWindow = function(type, index) {
        EditorQuizzesFactory.quizzesArr = $scope.quizzes[type];
        EditorQuizzesFactory.type = type;
        EditorQuizzesFactory.index = index;
        ...
        var url = domainName + "/v1/admin/news/quiz/" + $scope.quizzes[type][index]._id;
        $http.get(url).then(function successCallback(response){
            EditorQuizzesFactory.quiz = response.data.quiz;
            EditorQuizzesFactory.quizQuestionList = response.data.questionList;
            editorData.lastTabOpened = $scope.visibleTab;
            $location.path('/editor/quiz/edit/news');
        }, function errorCallback(response){
            console.log(JSON.stringify(response));
            alert("Unable to fetch data. Check internet.");
        });
...
app.controller('EditorQuizEditController', ['$scope', '$routeParams', '$http', 'domainName', 'EditorQuizzesFactory', '$window', function($scope, $routeParams, $http, domainName, EditorQuizzesFactory, $window) {
    $http.defaults.withCredentials = true;
    console.log("quiz : " + JSON.stringify(EditorQuizzesFactory.quizQuestionList));
    $scope.type = EditorQuizzesFactory.type;
    
    var quiz = EditorQuizzesFactory.quiz;
    var questions = EditorQuizzesFactory.quizQuestionList;
    $scope.quizId = quiz._id;
    $scope.quizName = quiz.nickname;
    $scope.quizType = quiz.type;
    $scope.questions = questions;
    $scope.quizPublishDate = quiz.publishDate;
    $scope.quizLength = questions.length;
    $scope.difficultyIndices = [{text: "Not Specified", value: 0}, {text: "Easy", value: 1}, {text: "Medium", value: 2}, {text: "Difficult", value: 3}];
    ...
    function questionReset() {
        ...
        $scope.question = {
            ...
            correctAnswer: {
                english: correct_answer.english,
                hindi: correct_answer.hindi
            },
            level: question.level,
            solution : {
                english: solution.english,
                hindi: solution.hindi
            }
        };
        $scope.quesLang = "english";
    };
    
    $scope.questionReset = questionReset;
    ...
    $scope.save = function(direction) {
        ...
        $http.put(url, data, config).then(function successCallback(response){
            ...
            question.level = $scope.question.level;
    ...
    $scope.approveQuiz = function() {
        if(!$scope.question.approved.english || !$scope.question.approved.hindi) {
            alert("Tick both the checkboxes(in hindi as well as english)");
            return;
        }
        $scope.$parent.loading = true;
        var content = {};
        ...
        $http.put(url, data, config).then(function successCallback(response){
            ...
            for(var i=0; i<$scope.questions.length; i++) {
                if($scope.questions[i].status != 'approved') {
                    $scope.$parent.loading = false;
                    alert("First approve all the questions. Then you can approve the quiz");
                    return;
                }
            }
            ...
            $http.put(url, data, config).then(function successCallback(response) {
                EditorQuizzesFactory.quizzesArr.splice(EditorQuizzesFactory.index, 1);
                simonGoBack();
                $scope.$parent.loading = false;
            }, function errorCallback(response) {
                $scope.$parent.loading = false;
